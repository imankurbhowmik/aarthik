...
const Income = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/api/incomes/${editingId}`, form, {
        headers: { Authorization: `Bearer ${token}` },
      });
      } else {
      ...
      fetchIncomes();
      setForm({ amount: "", source: "", description: "", date: dayjs().format("YYYY-MM-DD") });
      setEditingId(null);
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      {/* Income List */}
      <div className="mt-8">
        <h3 className="text-xl font-semibold mb-4">Your Incomes</h3>
        {incomes.length === 0 ? (
                ...
                {incomes.map((exp) => (
                  <tr key={exp._id} className="border-t">
                    ...
                    <td className="px-4 py-2">
                      {sources.find((c) => c._id === exp.source)?.name || "Unknown"}
                    </td>
